...
const app = Fastify({ logger: { redact: ["req.headers.authorization", "password", "password_hash"] }, bodyLimit: 1_000_000 });
const pool = new pg.Pool({ connectionString: env.DATABASE_URL, max: 20, ssl: env.NODE_ENV === "production" ? { rejectUnauthorized: true } : false });
...
app.post("/auth/register", { config: { rateLimit: { max: 8, timeWindow: "15 minutes" } } }, async (request, reply) => {
  ...
  try {
    ...
    return reply.code(201).send({ user: result.rows[0], token: await tokenFor(result.rows[0].id) });
  } catch (error) {
...
app.post("/auth/login", { config: { rateLimit: { max: 10, timeWindow: "15 minutes" } } }, async (request, reply) => {
  ...
  return { user: { id: user.id, email: user.email }, token: await tokenFor(user.id) };
});
...
app.post("/api/pages", async (request, reply) => {
  const userId = await auth(request);
  const input = z.object({ title: z.string().trim().min(1).max(300).default("Página sem título"), parentId: z.string().uuid().nullable().optional(), icon: z.string().max(8).nullable().optional(), content: z.array(z.unknown()).default([]) }).parse(request.body);
  const result = await pool.query("INSERT INTO pages(owner_id,parent_id,title,icon,content) VALUES($1,$2,$3,$4,$5) RETURNING *", [userId, input.parentId ?? null, input.title, input.icon ?? null, JSON.stringify(input.content)]);
  return reply.code(201).send(result.rows[0]);
});
app.patch("/api/pages/:id", async (request, reply) => {
  const userId = await auth(request);
  const id = z.string().uuid().parse((request.params as { id: string }).id);
  const input = z.object({ title: z.string().trim().min(1).max(300), content: z.array(z.unknown()), isFavorite: z.boolean() }).partial().parse(request.body);
  const result = await pool.query(`UPDATE pages SET title=COALESCE($1,title),content=COALESCE($2,content),is_favorite=COALESCE($3,is_favorite),updated_at=now() WHERE id=$4 AND owner_id=$5 RETURNING *`, [input.title ?? null, input.content ? JSON.stringify(input.content) : null, input.isFavorite ?? null, id, userId]);
...
app.post("/api/files", async (request, reply) => {
  ...
  try {
    ...
    const result = await pool.query("INSERT INTO files(owner_id,original_name,storage_name,mime_type,size_bytes) VALUES($1,$2,$3,$4,$5) RETURNING id,original_name,mime_type,size_bytes,created_at", [userId, part.filename.slice(0, 500), storageName, part.mimetype, size]);
    return reply.code(201).send(result.rows[0]);
  } catch (error) {
