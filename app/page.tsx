...
export default function Home() {
  ...
  useEffect(() => {
    ...
    saveTimer.current = window.setTimeout(async () => {
      ...
      // Salva os documentos preservando metadados e estado cifrado
      window.localStorage.setItem("orbe-documents", JSON.stringify(documents.map(({ file: _file, objectUrl: _url, ...item }) => {
        // Se a nota for segura e estiver bloqueada, nunca serializa texto em claro
      ...
      if (syncMode === "cloud" && session) {
        try {
          const content: CloudPage["content"] = [
            { type: "markdown", data: { markdown: note } },
            {
              type: "documents",
              data: {
                items: documents.map(({ file: _file, objectUrl: _url, ...item }) => {
                  if (item.security?.isLocked) {
                    return { ...item, content: undefined };
                  }
                  return item;
